...
# Make name list
list_of_wants <- read.csv("data/list_of_wants.csv", stringsAsFactors=FALSE)
...
getidigbio <- function(synonyms_list){
        ...
        #spocc_idigbio_reduced <- as.data.frame(spocc_idigbio_reduced)
...
getbison <- function(spocc_query){
        ...
        spocc_bison_reduced <- select(spocc_bison, tidyselect::all_of(bison_fields_all)) 
        ...
        if(bison_level == 0){
              ...
              if(nrow(spocc_query_df) == 0 & nrow(query_idigbio) == 0){
                ...
                spocc_query <- occ(query = synonyms_list,
                                   ...
                                   has_coords = FALSE, throw_warnings = FALSE)
                spocc_query_df <- occ2df(spocc_query)
